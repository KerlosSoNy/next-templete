...
const toastSlice = createSlice({
  ...
  reducers: {
    enqueueToast(state, action: PayloadAction<Omit<ToastType, 'id'>>) {
      ...
      state.queue.push({ id, message, type,title });
    },
    showNextToast(state) {
...
export const showToast = (title:string, message: string, type: ToastType['type']) => (dispatch: AppDispatch, getState: () => RootState) => {
  dispatch(enqueueToast({title, message, type }));
