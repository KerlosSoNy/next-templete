...
import { AttributeOption, ProductResponse } from '../types';
...
export const useProductBanner = ({ product, setSelectedProduct, selectedProduct,setMainProduct }: UseProductBannerProps) => {
    ...
    const [selectedColor, setSelectedColor] = useState<string | null | undefined>(undefined);
    ...
    const handleDecrement = () => {
        if (count > 1) {
            ...
            dispatch(decreaseCount({
                ...
                quantity: count,
                category:product?.category?.name|| 'فئــة المسكارة',
                price: selectedProduct?.price?.current_price ||120,
                name: product?.name || "مسكارا كثافة فائقة ",
                currency: selectedProduct?.price?.currency_symbol,
                image: selectedProduct?.product_variant?.featured_thumb_image_url || '',
                color: selectedColor || "#D89CB6",
                fit: selectedFit,
                size: selectedSize,
    ...
    const handleFavorite = async (id: number) => {
        await axiosInstance.post(`/${process.env.NEXT_PUBLIC_API_APP_TYPE}/${process.env.NEXT_PUBLIC_API_VERSION}/favorites`, {
            object_id: id,
...
// selectedProduct?.supplier_product?.is_favorite
    const handleFavoriteClick = () => {
        if (Token) {
            ...
            dispatch(showToast(t('toast.error'),t(`login_first`), 'error'));
        }
    };
    ...
    useEffect(() => {
        const cartItem = cart.find((item: any) => {
            return (
                ...
                // Match including null/undefined
                // (item.color === selectedColor || (!item.color && !selectedColor)) &&
                (item.size === selectedSize || (item.size == undefined && selectedSize == null))
            );
        });
