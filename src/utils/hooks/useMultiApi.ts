    ...
    const fetchData = async (endpointName: string, config: EndpointConfig | Record<string, any>, customPayload?: any) => {
        ...
        try {
            ...
            const response = await axiosInstance({
                method,
                url: `/${process.env.NEXT_PUBLIC_API_APP_TYPE}/${process.env.NEXT_PUBLIC_API_VERSION || version}/${endPoint}`,
                data: payload,
                headers: {
                    "page-key": page_key,
...
// Example For Usage 
// const { returnedData, errors, loading, fetchData, initializeEndpoints } = useMultiApi();
// 
